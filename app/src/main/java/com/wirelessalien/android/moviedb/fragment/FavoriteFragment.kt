...
class FavoriteFragment : BaseFragment() {
    ...
    override fun doNetworkWork() {
        if (!mShowListLoaded) {
            FavoriteListThread(mListType, 1).start()
        }
    }
                ...
                if (dy > 0 && recyclerView.scrollState != RecyclerView.SCROLL_STATE_IDLE) { // Check for scroll down and if user is actively scrolling.
                    ...
                    if (!loading && visibleItemCount + pastVisibleItems + threshold >= totalItemCount) {
                        ...
                        if (mShowArrayList.size > 0) {
                            FavoriteListThread(mListType, currentPage.toString().toInt()).start()
                        }
                        loading = true
                        currentPage++
                    }
    ...
    private inner class FavoriteListThread(private val listType: String?, private val page: Int) :
        Thread() {
        ...
        override fun run() {
            ...
            val url =
                "https://api.themoviedb.org/4/account/$accountId/$listType/favorites?page=$page"
