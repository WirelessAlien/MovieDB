...
class SearchPersonPagingSource(
    ...
    private suspend fun fetchSearchResultsFromApi(page: Int, query: String): String? {
        return withContext(Dispatchers.IO) {
            try {
                val url = URL("https://api.themoviedb.org/3/search/person?query=$query&page=$page&api_key=$apiKey${BaseActivity.getLanguageParameter(context)}${BaseActivity.getAdultContentParameter(context)}")
