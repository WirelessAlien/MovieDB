...
class PersonPagingAdapter : PagingDataAdapter<JSONObject, PersonPagingAdapter.PersonItemViewHolder>(
    PersonDiffCallback
) {
        ...
        if (personData != null) {
            ...
            try {
                ...
                if (personData.getString("profile_path") == null) {
                    holder.personImage.setBackgroundColor(ResourcesCompat.getColor(context.resources,
                        R.color.md_theme_outline, null))
                } else {
                    Picasso.get().load(
    ...
    object PersonDiffCallback : DiffUtil.ItemCallback<JSONObject>() {
        override fun areItemsTheSame(oldItem: JSONObject, newItem: JSONObject): Boolean {
            return try {
                oldItem.getInt("id") == newItem.getInt("id")
